...
def main_func():  
    ...
    try:
        ...
        os.makedirs(outputPath)
        showinfo(message='輸入值錯誤 (請手動刪除根據"輸入值"自動產生(不必要)的空目錄)!! '+' 建立收費單檔案存放目錄為 '+outputPath)    
    ...
    ##
    # 結束製作收費單功能
    showinfo(message='收費單已經好了! 檔案 請查閱  ' + outputPath + '  收費單.xlsx、收費單.pdf')
    stop()
#
# end of main_func()
##
...
class Update_Clock():
    def __init__(self, rootframe):
        self.root = rootframe
        self.label = tk.Label(self.root, text='mm/dd/yy 00:00:00', bg='#DDDDD8', 
...
def GUI():
    ...
    informationbar_lbl1 = tk.Label(div_4, text='版本 V.1.0', bg='#DDDDD8', font=fontStyle_2) #, bg='yellow', fg='white')
    ...
    #informationbar_lbl3 = tk.Label(div_4, text='mm/dd/yy 00:00:00', bg='#DDDDD8', font=tkFont.Font(family="Arial", size=10)')
