...
def convert_to_pdf(sourceFile='收費單_合併.xlsx', targetFile='收費單_合併.pdf'):
    ###
...
def merged_payment_list(noMerge=3, sourceFile='收費單.xlsx', targetFile='收費單_合併.xlsx'):
    ...
    name_list = df_student['姓名'] # 取出所有收費學生姓名清單   
    length = len(name_list) # 學生數(收費單數目)
        ...
        for idx in range(noMerge):
            ...
            for index_r, row in enumerate(ws.rows, start=1):
                ...
                if index_r == 1:
                    ...
                    ws_target.row_dimensions[r].height = 30
                elif (index_r >= 2) and (index_r <= itemRow+2): # '收費項目'數字+1(表格之表頭佔1列)，再+1(加上學生年級、班、姓名)
                    # 畫收費表格
                    for k in range(1, ws.max_column+1, 1): # 設定每個儲存格(cell)的格式
                        ws_target.cell(row=r, column=k).border = style_head['border']
                    ...
                    # 註記事項
                    ws_target.merge_cells(start_row=r, start_column=1, end_row=r, end_column=ws.max_column)
                    ws_target.cell(row=r, column=1).font = style_content['font']
                    ws_target.cell(row=r, column=1).alignment = style_content['alignment']
                    ws_target.row_dimensions[r].height = 15                            
            # update row index
            rowIdx = r + 1 # 加入1列空白於兩個學生收費單之間 #rowIdx + index_r+1
            # 合併學生的收費單後，將單一學生的收費單(工作表)移除，只保留合併後收費單(工作表)
    ...
    for idx in range(length % noMerge):
        ...
        # update row index
        rowIdx = r + 1 # adding one row used as separation between two student paymentsheets 
        # remove single worksheet of each student
        wb_source.remove(wb_source[sheetName])
    
    ##
    # finally, 產出合併收費單(Excel worksheet以及PDF
    wb_source.save(targetFile) # Excel file
    wb_source.close()
    convert_to_pdf(sourceFile='收費單_合併.xlsx', targetFile='收費單_合併.pdf') # PDF file    
#
...
if __name__ == '__main__':  
    ...
    ##
    # ending
    print('好了! 請看 收費單.xlsx 收費單_合併.xlsx 收費單_合併.pdf')
#
# end of main()
##