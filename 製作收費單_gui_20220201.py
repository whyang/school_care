...
def main_func():  
    ...
    try:
        ...
        os.makedirs(outputPath)
        showinfo(
            title = '收費資料 及 收費單存放目錄',
            message =
            '輸入值錯誤 (請手動刪除根據"輸入值"自動產生(不必要)的空目錄)!! '+' 建立收費單檔案存放目錄為 ' +
            outputPath)    
...
def GUI():
    ...
    input_pay_list_path.set('收費單_'+data_time)
    ...
    informationbar_lbl1 = tk.Label(div_4, text='版本 V.2.0', bg='#DDDDD8', font=fontStyle_2) #, bg='yellow', fg='white')
